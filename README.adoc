...
:page-seo-title: Enabling distributed tracing in Java microservices using Eclipse MicroProfile OpenTracing and Jaeger
...
Set the following environment variables to sample all traces:
include::{common-includes}/os-tabs.adoc[]
[.tab_content.mac_section.linux_section]
--
[role='command']
...
The `const` value for `JAEGER_SAMPLER_TYPE` configures the Jaeger client sampler to make the same sampling decision,
based on the sampler parameter.
It either samples all traces (sampler parameter is 1) or none (sampler parameter is 0).

The `1` value for JAEGER_SAMPLER_PARAM` configures the Jaeger sampler to sample all traces.
...
Check https://github.com/jaegertracing/jaeger-client-java/tree/master/jaeger-core#configuration-via-environment[here^]
for a list of all configuration environment variables.

Since the environment variables configure the Jaeger client, these environment variables must be configured in the
command-line sessions where you will start the Open Liberty server.
...
Verify that the Jaeger client configuration environment variables are set in this command-line session.
