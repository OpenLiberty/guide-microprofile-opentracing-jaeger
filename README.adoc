...
:page-description: Explore how to enable and customize tracing of RESTful API and non-RESTful API methods by using Jaeger and MicroProfile OpenTracing.
...
Explore how to enable and customize tracing of RESTful API and non-RESTful API methods by using MicroProfile OpenTracing and Jaeger.
...
You will learn how to enable automatic tracing for RESTful API methods and create custom tracers for non-RESTful API methods by using MicroProfile OpenTracing.
...
The https://github.com/eclipse/microprofile-opentracing[MicroProfile OpenTracing feature^] enables tracing of all RESTful API methods by default. To further control and customize these traces, use the `@Traced` annotation to enable and disable tracing of particular methods. You can also inject a custom `Tracer` object to create and customize spans.
...
Because tracing of all RESTful API methods is enabled by default, you only need to enable the [hotspot=mpOpenTracing file=0]`MicroProfile OpenTracing` feature in the `server.xml` file to see some basic traces in Jaeger.
...
* The `value=[true|false]` parameter indicates whether a particular class or method is traced. For example, while all RESTful API methods are traced by default, you can disable their tracing by using the `@Traced(false)` annotation. This parameter is set to `true` by default.
...
Enable tracing of the [hotspot=list file=0]`list()` non-RESTful API method by updating [hotspot=Traced file=0]`@Traced` as shown.
...
You see a new trace record that is two spans long. One span is for the [hotspot=listContents file=1]`listContents()` RESTful API method in the [hotspot file=1]`InventoryResource` class, and the other span is for the [hotspot=list file=0]`list()` method in the [hotspot file=0]`InventoryManager` class.
...
You can use the `@Traced` annotation with a value of `false` to disable automatic distributed tracing of RESTful API methods.
...
[role="edit_command_text"]
Disable tracing of the [hotspot=listContents file=1]`listContents()` RESTful API method by setting [hotspot=Traced-false file=1]`@Traced(false)`.
