...
:page-description: Explore how to enable and customize tracing of JAX-RS and non-JAX-RS methods by using Jaeger and MicroProfile OpenTracing.
:page-tags: ['MicroProfile', 'OpenTracing', '@Traced']
...
The `Trace` type in OpenTracing can be thought of as a directed acyclic graph (DAG) of `Span`s, where the edges between
`Span`s are called References.
The `Tracer` interface creates `Span`s and understands how to serialize and deserialize their metadata across process
boundaries.

MicroProfile OpenTracing enables distributed tracing in microservices. Note that the MicroProfile OpenTracing
specification does not address the problem of defining, implementing, or configuring the underlying distributed tracing
system. Rather, the specification makes it easy to instrument services with distributed tracing given an
...
Before you begin, deploy the Jaeger all-in-one executable to start the Jaeger tracing system.
The Jaeger all-in-one executable is configured for quick local testing.
You can find information about the Jaeger server and instructions for starting the all-in-one executable in their
https://www.jaegertracing.io/docs/1.17/getting-started/#all-in-one[documentation^].
...
In a development environment like this guide, it is important that every trace is sampled.
...
OpenLiberty [hotspot=jaegerClientConfig]`configuration properties` to sample all traces.

The `const` value for [hotspot=samplerTypeConfig]`JAEGER_SAMPLER_TYPE` configures the Jaeger client sampler to make the
same sampling decision, based on the sampler parameter.
It either samples all traces (sampler parameter is 1) or none (sampler parameter is 0).

The `1` value for [hotspot=samplerParamConfig]`JAEGER_SAMPLER_PARAM` configures the Jaeger sampler to sample all traces.

In a production environment, this configuration could cause a lot of overhead on the application and a lower sampling
rate can be used. The different values for client sampling configuration can be found in the
https://www.jaegertracing.io/docs/1.18/sampling/#client-sampling-configuration[sampling^] documentation.

Similarly, in a production environment, Jaeger may not be running in the same host as the application.
In this case, you must set the hostname of the Jaeger server to the `JAEGER_AGENT_HOST` environment variable and
the port for communicating with the Jaeger host to the `JAEGER_AGENT_PORT` environment variable.

Check out all configuration environment variables at the
https://github.com/jaegertracing/jaeger-client-java/tree/master/jaeger-core#configuration-via-environment[Jaeger Java client^]
documentation.
...
OpenTracing API is exposed as a third party API in Liberty.
To add the visibility of OpenTracing APIs to the application, `third-party` must be added to the types of API packages
...
set to the [hotspot=thirdParty]`apiTypeVisibility` attribute of the [hotspot=thirdParty]`<classLoader />` configuration
to add third-party to the default list of API package types supported.
