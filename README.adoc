...
https://www.jaegertracing.io/docs/1.22/getting-started/#all-in-one[Jaeger documentation^].
...
to the [hotspot=thirdParty]`apiTypeVisibility` attribute in the [hotspot=thirdParty]`<classLoader />` configuration.
...
// static guide instructions:
ifndef::cloud-hosted[]
Go to the {inv-url}[{inv-url}^] URL and check your Jaeger server at the {jaeger-url}[{jaeger-url}^] URL.
...
[source, Java, linenums, role='code_column hide_tags=copyright,addSpan,endTry Add tags=addToInvList']
----
