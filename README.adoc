...
:page-description: Explore how to enable and customize tracing of RESTful Web Services and non-RESTful Web Services methods by using Jaeger and MicroProfile OpenTracing.
...
The https://github.com/eclipse/microprofile-opentracing[MicroProfile OpenTracing feature^] enables tracing of all restfulWS methods by default. To further control and customize these traces, use the `@Traced` annotation to enable and disable tracing of particular methods. You can also inject a custom `Tracer` object to create and customize spans.
...
Because tracing of all restfulWS methods is enabled by default, you only need to enable the [hotspot=mpOpenTracing file=0]`MicroProfile OpenTracing` feature in the `server.xml` file to see some basic traces in Jaeger.
...
Enable tracing of the [hotspot=list file=0]`list()` non-restfulWS method by updating [hotspot=Traced file=0]`@Traced` as shown.
...
You see a new trace record that is two spans long. One span is for the [hotspot=listContents file=1 hide_tags=Traced-false]`listContents()` restfulWS method in the [hotspot file=1]`InventoryResource` class, and the other span is for the [hotspot=list file=0]`list()` method in the [hotspot file=0]`InventoryManager` class.
...
----
[role="edit_command_text"]
Disable tracing of the [hotspot=listContents file=1]`listContents()` restfulWS method by setting [hotspot=Traced-false file=1]`@Traced(false)`.
