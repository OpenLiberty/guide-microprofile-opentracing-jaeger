...
https://www.jaegertracing.io/[Jaeger^] is an open source distributed tracing system that is compatible with the
...
Before you begin, deploy the Jaeger all-in-one executable file to start the Jaeger tracing system.\
The Jaeger all-in-one executable file is configured for quick local testing.
You can find information about the Jaeger server and instructions for starting the all-in-one executable file in their
...
To view the traces, go to the {jaeger-url}[{jaeger-url}^] URL.
...
Open Liberty [hotspot=jaegerClientConfig]`configuration properties` to sample all traces.
...
The `1` value for [hotspot=samplerParamConfig]`JAEGER_SAMPLER_PARAM` variable configures the Jaeger sampler to sample
all traces.
...
https://github.com/jaegertracing/jaeger-client-java/tree/master/jaeger-core#configuration-via-environment[Jaeger Java client documentation^].
...
The https://github.com/eclipse/microprofile-opentracing[MicroProfile OpenTracing feature^] enables tracing of all JAX-RS methods by default.
...
Because tracing of all JAX-RS methods is enabled by default, you only need to enable
[hotspot=mpOpenTracing file=0]`MicroProfile OpenTracing` feature in the `server.xml` file
to see some basic traces in Jaeger.

OpenTracing API is exposed as a third party API in Open Liberty.
...
Instead of explicitly configuring a list of API packages which includes `third-party`, set the `+third-party` value
to the [hotspot=thirdParty]`apiTypeVisibility` attribute of the [hotspot=thirdParty]`<classLoader />` configuration.
...
If you omit this parameter, the span will is named with the `<package name>.<class name>.<method name>` format.
...
Go to the {inv-url}[{inv-url}^] URL and check your Jaeger server at {jaeger-url}[{jaeger-url}^].
...
[hotspot=listContents file=1 hide_tags=Traced-false]`listContents()` JAX-RS method in the
[hotspot file=1]`InventoryResource` class and the other span is for the [hotspot=list file=0]`list()`
...
This [hotspot=Try file=0]`try` block is called a `try-with-resources` statement, meaning that the
...
Go to the {inv-url}/localhost[{inv-url}/localhost^] URL and check your Jaeger server at {jaeger-url}[{jaeger-url}^].
If you have the Jaeger UI open from a previous step, refresh the page.
Select the `inventory` traces and click the **Find Traces** button.
...
Nice work! You just used MicroProfile OpenTracing in Open Liberty to customize how and which traces are delivered to
Jaeger.
